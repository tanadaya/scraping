...
def parallel_scraping(llino_list, max_workers=1, driver_opts=None, config=None):
    ...
    if not targets:
        return _impl.parallel_scraping(targets, max_workers=max_workers, driver_opts=driver_opts, config=config)
    ...
    description = f"Movement: {cfg.get('period', 'all')}"
    with coordinated_job(
        ...
        with track_unique_function_calls(
            ...
            return _impl.parallel_scraping(
                targets,
...
class _ImplementationProxy(types.ModuleType):
    """Delegate non-facade attributes (including test monkeypatches) to impl."""
...
if "parallel_scraping" not in __all__:
    __all__.append("parallel_scraping")
